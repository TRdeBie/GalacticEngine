...
//Spotlight Function
float4 SpotLight(float4 Position3D, int i)
{
	...
	float cosAlpha = max(0.0f, dot(directionToLight, LightDirection[i]));
...
float4 CookTorrancePortion(float3 N, float3 V, float m2, float F, uint i, float3 Light)
{
	...
	float4 color = float4(SpecularIntensity * dot(N, L) * (BaseColor[i] * Rs), 1.0f); //Instead of using white as a specular colour, the light's own colour is chosen
...
{	
	...
	//CookTorrance pre-calculations: to avoid having to redo these calculations
	float3 N = newNormal;
	...
	for(uint i = 0; i < MAX_LIGHTS; i++)
	{
		...
		attenuation = SpotLight(input.Position3D, i);
		color = color + AmbientPortion() * attenuation; //Calculate the ambient portion once
		color = color + (LambertPortion(newNormal, i, RelativeLight) * attenuation);
		color = color + (CookTorrancePortion(newNormal, V, m2, F, i, RelativeLight) * attenuation); 

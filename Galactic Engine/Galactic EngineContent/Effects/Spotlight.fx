...
{	
	float attenuation = SpotLight(input.Position3D); //Get the attenuation (as indication for what is in and out of the spotlight)

	float4 baseColor = float4(0, 0, 0, 1); //Declare a base color
	float3x3 rotationAndScale = (float3x3) InverseTransposeWorld; //Get the proper normals with the inversetransposeworld-matrix
	float3 newNormal = mul(input.Normal, rotationAndScale);
	newNormal = normalize(newNormal);

	//Build up the final colour with the ambient base colouring, lambertian portion, Cook-Torrance highlights and specular highlights
	//Each part is multiplied with attenuation to ensure that only parts within the spotlight get coloured
	//For the ambient part, 0.3f is added so that shapes are still visible in the shadows as well
	baseColor = baseColor + (attenuation + 0.3f) * AmbientPortion();
	if (attenuation > 0) //As it makes little sense to calculate the portions, only to multiply them by 0, this if-statement is implemented
	{
		baseColor = baseColor + attenuation * LambertPortion(newNormal);
